...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if 'g' in args.generator_arch and 'd' in args.discriminator_arch:
        ...
        warnings.warn('You have chosen a specific model architecture. '
                      'This will default use MNIST model architecture and '
    ...
    for epoch in range(args.start_epoch, args.epochs):
        # train for one epoch
        train(dataloader, generator, discriminator, adversarial_loss,
              optimizerG, optimizerD, g_losses, d_losses, epoch, args)
...
def train(data_loader, generator, discriminator, adversarial_loss, optimizer_g,
          optimizer_d, g_losses, d_losses, epoch, args):
    global fixed_noise
    ...
    for i, data in progress_bar:
        ...
        progress_bar.set_description(
            f"[{epoch}/{args.epochs - 1}][{i}/{len(data_loader) - 1}] "
...
def validate(model, args):
    ...
    with torch.no_grad():
        ...
        if args.gpu is not None:
            noise = noise.cuda(args.gpu, non_blocking=True)
        fake = model(noise)
        vutils.save_image(fake.detach(), f"{args.outf}/fake.png",
                          normalize=True)
